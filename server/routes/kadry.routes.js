        ...
        try {
            ...
            if (sqlProv[0].length === 0) {
                ...
                if (telNumber === '') {
                    ...
                    try {
                        await fs.promises.access(`../server/files/kadry/QuestionnaireApplicants/${telNumber}/`);

                        console.log("папка существует")
                        fs.mkdir(`../server/files/kadry/QuestionnaireApplicants/${telNumber}`, {recursive: true}, err => {
                            if (req.files != null) {
                                file.mv(`../server/files/kadry/QuestionnaireApplicants/${telNumber}/` + telNumber + `.` + file.name.split(".").slice(-1));
                            }
