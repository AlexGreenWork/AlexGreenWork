...
router.post("/toDbSaveResponsible", urlencodedParser, authMiddleware,
    async function (request, response) {
        ...
        let sqlCheckoffResponsible = await pool.query(`SELECT offer_id  FROM offersresponsible WHERE responsible_tabnum=${respTabnum} AND offer_id=${idOffers} AND deleted = 0` ) 
        console.log(sqlCheckoffResponsible[0].length)
      
		try
		{
            if(sqlCheckoffResponsible[0].length == 0){
...
router.post("/responsibleToOffers", urlencodedParser,
    async function (request, response) {
        ...
        let sqlResponsible_Rg = await pool.query(`SELECT offer_id, close  FROM offersresponsible_rg WHERE responsible_tabnum=${tabNum} AND deleted = 0 `);
        
         let resPonsibleAll = sqlResponsible[0].concat(sqlResponsible_Rg[0])
         
        for(let i = 0; i < resPonsibleAll.length; i++){
      
            let sqlOffers = await pool.query(`SELECT Id  FROM offers WHERE Id=${resPonsibleAll[i].offer_id} `)
           
            if(sqlOffers[0].length != 0){
               
                arrValidOffers.push(resPonsibleAll[i])
                if(resPonsibleAll[i].close == null){
                    count_resp_no_close ++;
                    let objUnlockOffers = {};
                    objUnlockOffers[resPonsibleAll[i].offer_id] =  true;
                    unlockOffers.push(objUnlockOffers)
                } else {
                    // console.log(true)
                    let objUnlockOffers = {};
                    objUnlockOffers[resPonsibleAll[i].offer_id] =  false
                    unlockOffers.push(objUnlockOffers)
                }
            }
...
router.post("/getHistoryBrowsing", urlencodedParser,
    async function (req, res) {
        ...
        let tabNum = req.body.tabNum;
        let sqlHistBrows = await pool.query(`SELECT id_offers FROM browsing_history WHERE tabNum = '${tabNum}'`)
        let alloffers = await pool.execute(`SELECT
														o.nameOffer,
														o.Id,
														o.date,
														o.status,
        ...
        countTrue = 0
        for(let i = 0; i < sqlHistBrows[0].length; i++){
            for(let j = 0; j < alloffers[0].length; j++){
                if(sqlHistBrows[0][i].id_offers == alloffers[0][j].Id){
                    countTrue++
                }
            }
        }

        if( alloffers[0].length - countTrue >= 0){
            res.send([String(alloffers[0].length - countTrue), sqlHistBrows[0]]);
        } else {
            res.send([0, sqlHistBrows[0]]);
        }

        pool.end()
    })

router.post("/editTextOffer", urlencodedParser,
    async function (req, res) {
        const mysqlConfig = {
            host: config.database.host,
            user: config.database.user,
        ...
        const pool = mysql.createPool(mysqlConfig);
        let textOffer = req.body.textOffer
        let tabNum = req.body.tabNum
        let idOffers = req.body.idOffers
        var date = moment().format('MMMM Do YYYY, h:mm:ss a');
        try {
            let oldTextOffer =   await pool.query(`SELECT textOffer FROM  offers WHERE  tabelNum = '${tabNum}' AND Id = '${idOffers}'`)
            let old = '';
            let count = 0;

            for(let i = 0; i < Math.ceil(oldTextOffer[0][0].textOffer.length/100); i++){

                old = old +  oldTextOffer[0][0].textOffer.substring(count, count +100) + '\r\n'
                count = count + 100
            }

            try {
                await pool.query(`UPDATE offers SET textOffer = '${textOffer}' WHERE  tabelNum = '${tabNum}' AND Id = '${idOffers}'`)
            } catch (e) {
                console.log(e)
            }

            fs.readdir(`../server/files/offers/OffersEditLogs/`, (err, folderList) => {
                console.log( folderList)
                if (folderList.includes(`id${idOffers}`) == false) {

                    fs.mkdir(`../server/files/offers/OffersEditLogs/id${idOffers}/`, { recursive: true }, err => {
                        if (err) throw err; // не удалось создать папкy

                        let a = "\r\n==========================================\r\n\r\n"

                        let toWrite = '\r\n' + "edit editTextOffer" +'\r\n' +JSON.stringify(date)  +'\r\n\r\n' + "ip:\r\n  " + req.ip + '\r\n\r\n' +"edit tab: "+ tabNum+ "old content:\r\n\r\n" + old + a;

                        fs.writeFile(`../server/files/offers/OffersEditLogs/id${idOffers}/log.txt`, toWrite, function (error) {
                    ...
                    fs.readFile(`../server/files/offers/OffersEditLogs/id${idOffers}/log.txt`, 'utf8', function(error, fileContent){
            ...
            console.log("изменение предложения", req.body)
            res.send("true");
        }catch(e){
            console.log(e)
            res.send("false");
...
router.post("/editPropblem", urlencodedParser,
    async function (req, res) {
        const mysqlConfig = {
            host: config.database.host,
            user: config.database.user,
        ...
        var date = moment().format('MMMM Do YYYY, h:mm:ss a');
        try {
            let oldProblem =   await pool.query(`SELECT descriptionProblem FROM  offers WHERE  tabelNum = '${tabNum}' AND Id = '${idOffers}'`)

            let old = '';
            let count = 0;
            for(let i = 0; i < Math.ceil(oldProblem[0][0].descriptionProblem.length/100); i++){

                old = old +  oldProblem[0][0].descriptionProblem.substring(count, count +100) + '\r\n'
                count = count + 100
            }


            try {
                await pool.query(`UPDATE offers SET descriptionProblem = '${problem}' WHERE  tabelNum = '${tabNum}' AND Id = '${idOffers}'`)
            } catch (e) {
                console.log(e)
            }
