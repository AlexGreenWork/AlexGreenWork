    ...
    if (viewChange == false) {
      return (
        ...
        >
          ...
          >
            Выбрать сотрудника рабочей группы
          </Button>
        </div>
      );
    }
...
console.log(responsibles)
    ...
    return (
      <div>
        <Button
          ...
          onClick={addResp}
        >
          + Добавить заключение и отвественного сотрудника
        </Button>
      </div>
    );
  }
  ...
  const [annotationRGMark, setAnnotationRGMark] = useState(
  ...
  function AdminChangeUploadFile(props) {
    ...
    if (
      isAdmin ==
      `${store.getState().offers.offer.responsibles_rg?.responsible_tabnum}`
    ) {
      return <FilesRG />;
      return <div></div>;
    } else {
  ...
  async function saveResponsible() {
    try {
      ...
      await server.send_post_request(
        `${API_URL}api/offers/toDbSaveResposibleRG`,
        {
          respTabnum,
          respName,
          idOffer,
  ...
  const [nameRG, setNameRg] = useState(
    `${store.getState().offers.offer.responsibles_rg?.fiofull}`
  );
  const [dateRG, setDateRg] = useState(
    `${store.getState().offers.offer.responsibles_rg?.open}`
  );
  const [dateCloseRG, setDateCloseRg] = useState(
    `${store.getState().offers.offer.responsibles_rg?.close}`
  );
  const [tabelRG, setTabelRg] = useState(
    `${store.getState().offers.offer.responsibles_rg?.responsible_tabnum}`
  );
  ...
  var newDate =
    open.getDate().toString().padStart(2, "0") + " " + month[open.getMonth()];

  var close = new Date(`${dateCloseRG}`);
  var newCloseDate =
    close.getDate().toString().padStart(2, "0") + " " + month[close.getMonth()];
    ...
    if (
      ...
      return (
        <div
          ...
          className={s.conclusionTextArea}
          style={{
            width: "100%",
          ...
          <Table aria-label="collapsible table">
            <TableHead>
              <TableRow>
                ...
                <TableCell align="right">Экономичность</TableCell>
                <TableCell align="right">Скорость внедрения</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              <TableCell />
              ...
              <TableCell
                ...
                type="number"
              >
                {store.getState().offers.offer.responsibles_rg?.innov}
              </TableCell>
              <TableCell
                id="cost"
                className={s.NoteCell}
                ...
                type="number"
              >
                {store.getState().offers.offer.responsibles_rg?.cost}
              </TableCell>
              <TableCell
                id="duration"
                className={s.NoteCell}
                ...
                type="number"
              >
                {store.getState().offers.offer.responsibles_rg?.extent}
              </TableCell>
            </TableBody>
          </Table>
          <div>
            <Button
              ...
              onClick={(value) => {
                saveNotes(
                  store.getState().offers.offer.responsibles_rg
                    ?.responsible_tabnum
                );
              }}
            >
      ...
      return (
        <TableContainer component={Paper}>
          <Table aria-label="collapsible table">
            ...
            <TableBody>
              <TableCell />
              <TableCell align="right" type="number">
                {store.getState().offers.offer.responsibles_rg?.actual}
              </TableCell>
              <TableCell align="right" type="number">
                {store.getState().offers.offer.responsibles_rg?.innov}
  ...
  function CloseChange(isClose) {
    ...
    if (isClose.isClose == "undefined") {
      ...
      return (
        ...
        >
          {" "}
          Заключение закрыто: {newCloseDate}
          {" " + close.getFullYear()} года.
        </div>
      );
    }
  }
    ...
    <div id="OffContainer" className={s.cardOfferContainer1}>
      ...
      >
          ...
          >
            ...
            >
              <div
                style={{
                  backgroundImage: `url(${
                    API_URL + "files/photos/" + tabelRG + ".jpg"
                  })`,
                  width: "40px",
                  height: "40px",
        ...
        <div className={s.filesConclusion}>
          ...
          >
            Файлы заключения рабочей группы:
          </div>
          <div className={s.conclusionFilesArea}>
            <div
              className={s.conclusionFilesAreaFile}
              style={{
                display: "flex",
                flexDirection: "column",
                cursor: "pointer",
          ...
          <AdminChangeUploadFile
            isAdmin={localStorage.getItem("userTabelNum")}
          />
        </div>
        <div className={s.filesConclusion11}>
          ...
          >
            Краткая аннотация заключения рабочей группы:
          </div>
          <AdminChangeUploadAnnotation
            isAdmin={localStorage.getItem("userTabelNum")}
          />
          <AdminChangeAnnotationWiev
            isAdmin={localStorage.getItem("userTabelNum")}
          />
        </div>
