...
const ConclusionCard = (props) => {
  ...
  if (
    props.id.mark == "null" ||
    (props.id.mark == null &&
      props.id.responsible_tabnum != localStorage.getItem("userTabelNum"))
  ) {
    props.id.mark = "";
  ...
  async function saveAnnot(idOffer, tabNum) {
    ...
    try {
      const ann = document.getElementById(
        `${"annotation" + props.name}`
      ).innerText;
      const position = props.name;
      console.log(
        props.id.responsible_tabnum,
        props.name,
        idOffer,
        tabNum,
        ann
  ...
  const CardDivisionConclusion = () => {
    ...
    async function saveNotesResponsible() {
      try {
        ...
        const innovate = document.getElementById(
          "innovate" + props.name
        ).innerText;
    ...
    function ConfirmResponsible() {
      ...
      if (
        ...
        return (
          <div>
            В обработке с {newDate}
            {" " + d.getFullYear()}...
            <Button
              onClick={(value) => {
                closeCunclusion(props.id.tabNum);
              }}
            >
              Завершить работу с заключением
            </Button>
          </div>
        );
      }
      ...
      if (
        ...
        return <div>Еще не принято в обработку</div>;
      } else {
        return <div></div>;
      }
    }
    ...
    var newCloseDate =
      close.getDate().toString().padStart(2, "0") +
      " " +
    ...
    return (
      ...
      >
        ...
        <div className={s.header}>
            ...
            >
              ...
              >
                <div
                  style={{
                    backgroundImage: `url(${
                      API_URL + "files/photos/" + props.tabel + ".jpg"
                    })`,
                    width: "40px",
                    height: "40px",
          ...
          {isActive && (
            <div className={s.conclusionAccordion}>
              <div className={s.filesConclusion11}>
                ...
                <AdminChangeSaveAnnotation
                  {...props}
                  isAdmin={localStorage.getItem("userAdminOptions")}
                />
                <ChangeAnnotViewContentEditable
                  isMy={localStorage.getItem("userTabelNum")}
                />
              </div>
              ...
              <div className={s.filesConclusion11}>
                ...
                <ViewNotesResposible
                  {...props}
                  isResponsible={localStorage.getItem("userTabelNum")}
                />
