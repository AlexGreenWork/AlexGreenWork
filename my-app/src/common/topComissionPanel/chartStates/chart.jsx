    ...
    function paint() {
        ...
        switch(property.type){
            case "linear":  const dataLinear = chartLinear(idElem, property, data, HEIGHT_DPI, WIDTH_DPI, VIEW_HEIGHT, yMin, yMax, yRatio, xRatio, xMax, xMin, VIEW_WIDTH, proxy.mouse)
                ...
                verticalLineValue(proxy.mouse.x, dataLinear, property.padding_top, xRatio, HEIGHT_DPI, WIDTH_DPI, xMax, xMin, sortObjdata, property.padding_left, yRatio, property.padding_right, data, property.intCircle, property.x_type, idElem, property)
...
function chartLinear(idElem, property, data, HEIGHT_DPI, WIDTH_DPI, VIEW_HEIGHT, yMin, yMax, yRatio, xRatio, xMax, xMin, VIEW_WIDTH, mouseX, CIRCLE_RADIUS) {
    ...
    for (let i = 0; i < keyData.length; i++) {
        ...
        if (property.textVertex.active === "true") {
            darwTextVertex(idElem, arrData, xRatio, property.padding_left, HEIGHT_DPI, property.padding_top, yRatio, property.textVertex.correctX_pos, property.textVertex.correctY_pos, property.textVertex.font, property.textVertex.text_color)
        }
...
function verticalLineValue(mouseX, dataX, padding_top, xRatio, HEIGHT_DPI, WIDTH_DPI, xMax, xMin, sortObjdata, padding_left, yRatio, padding_right, data , intCircle, x_type, idElem, property) {
    ...
    for (let i = 0; i < elemData.length; i++) {
        ...
        for (let j = 0; j < sortObjdata[elemData[i]].length; j++) {
            ...
            if (isOver(idElem, mouseX, x, sortObjdata[elemData[i]].length, y, WIDTH_DPI, HEIGHT_DPI, padding_left, padding_top, xRatio, yRatio, padding_right, mouse_circle, data[elemData[i]].dataName, color, intCircle, elemData[i], x_type, property) === true) {
                checkCirkle = true
                break;
            } else {
...
function darwTextVertex(idElem, arrData, xRatio, padding_left, HEIGHT_DPI, padding_top, yRatio, correctX_pos, correctY_pos, font, text_color) {
    var canvas = document.getElementById(idElem);
    const ctx = canvas.getContext("2d");
...
function isOver(idElem, mouse, x, length, y, WIDTH_DPI, HEIGHT_DPI, padding_left, padding_top, xRatio, yRatio, padding_right, mouse_circle, nameData, color, intCircle, elemData, x_type,property) {
